...
-- Creates all tables, ticket prices, seat layouts, and revenue view
...
-- Populate seat layouts for all existing auditoriums
...
SELECT ticket_type, price FROM ticket_prices;

-- Show seat counts per auditorium
SELECT 'Seats per Auditorium:' as info;
SELECT
  t.name as theater,
...
SELECT
  ...
  (SELECT COUNT(*) FROM seat_reservations) as total_reservations,
  (SELECT COUNT(*) FROM seat_layouts) as total_seats_defined,
  (SELECT COUNT(*) FROM ticket_prices) as ticket_types;
